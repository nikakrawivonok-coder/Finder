...
'use strict';
const VERSION='V35', STORE='anomalyArtifactGameV35', SIZE=8, FLAGS={"undo": true, "danger": true, "route": true, "events": true, "perks": true, "achievements": true, "soundtest": true};
...
{let prefs=loadPrefs();
let actionHelpOpen=false; if(prefs.perk)setup.perk=prefs.perk; if(typeof prefs.compact==='boolean')setup.compact=prefs.compact; if(prefs.audio)audioState={...audioState,...prefs.audio};}
...
function renderActionFeedback(){
  ...
  let d=state.actionDetail||{main:state.lastAction||'КПК очікує дію.',why:'Тут після кожної дії буде пояснення наслідку.',next:'Зроби перший обережний крок або кинь болт.'};
  ...
  p.innerHTML='<div class="impactTitle">'+esc(d.main)+'</div><div class="impactWhy">'+esc(d.why)+'</div><div class="impactNext">'+esc(d.next)+'</div>'+decisions;
}
...
function hideAll(){$$('#setupPanel,#gamePanel,#logPanel,#resultPanel,#archivePanel,#rulesPanel').forEach(e=>e.classList.add('hidden'))}
...
function renderActionGuide(){let g=$('#actionGuide'),box=$('#actionExplain');if(!g||!state)return;let msg='';
...
setupUI=function(){_origSetupUIV33();let hero=document.querySelector('.heroCall .small');if(hero)hero.textContent='V34 — Final Playtest Cut: фінальна версія перед перевіркою, профілі, автосейв, експорт сейву, м’який баланс і ясний зворотний зв’язок після дій.';renderProfilePanelV33();balanceNoteV33();};
...
const V34_PATCH_NOTE = 'V34 — Final Playtest Cut: фінальна підготовка до перевірки, м’якший баланс, сейв-профілі, експорт/імпорт прогресу, ясніша остання дія.';
...
setupUI = function(){
  _origSetupUIV34();
  document.title='Аномальне поле · V35 True Playtest Ready';
  let hero=document.querySelector('.heroCall .small');
  if(hero)hero.textContent=V34_PATCH_NOTE;
  let btn=$('#selfCheckBtn'); if(btn)btn.textContent='Самоперевірка V35';
...
showSelfCheck = function(){
  ...
  if(p){
    let title=p.querySelector('b'); if(title)title.textContent='Самоперевірка V35';
...
showArchive=function(){
  ...
  panel.insertAdjacentHTML('beforeend','<br><b>Checkpoint V35</b><div class="archiveList"><div class="archiveItem"><b>Final Playtest Cut</b><br>Ця версія створена для твоєї перевірки: профілі, автосейв, експорт/імпорт сейву, м’якший старт, чесніші повідомлення після дій, сенсорні кроки без прямих передбачень.</div></div>');
};

const _origExportCardV34 = exportCard;
exportCard=function(){let base=_origExportCardV34();return base+'\nВерсія тесту: V34 Final Playtest Cut\nСейв: профільний localStorage, доступний експорт/імпорт у меню профілю.';};
...
const V35_PATCH_NOTE = 'V35 — True Playtest Ready: останнє згладжування перед тестом, менше тертя в керуванні, швидка кнопка “Що робити зараз?” і чистіший старт.';
...
showArchive=function(){
  _origShowArchiveV35();
  let panel=$('#archivePanel'); if(!panel)return;
  panel.insertAdjacentHTML('beforeend','<br><b>Checkpoint V35</b><div class="archiveList"><div class="archiveItem"><b>True Playtest Ready</b><br>Це остання передтестова версія: не додає новий великий контент, а зменшує тертя — ситуаційна підказка, сховані складні дії, зрозуміший старт і чистіша самоперевірка.</div></div>');
};
